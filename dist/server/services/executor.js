...
export class AutomationExecutor {
     ...
     * @returns Resultado da execução
     */
    async executeQuickReply(automation, context) {
        try {
            ...
            const triggerConfig = (automation.trigger || {});
            ...
            const matchFound = keywords.some(keyword => incomingContent.includes(keyword.toLowerCase()));
            if (!matchFound) {
                return {
                    success: false,
            ...
            const responseTemplate = automation.response || '';
            ...
            if (context.channel) {
                await this.sendMessageViaChannel(context.channel, context.conversation.contactIdentifier, processedResponse);
            }
            return {
                success: true,
     ...
     * @returns Resultado da execução
     */
    async executeChatbot(automation, context) {
        try {
            // Verificar se há mensagem de entrada e conversa
            if (!context.incomingMessage || !context.conversation) {
                return {
            ...
            const modelConfig = (automation.modelConfig || {});
            // Obter histórico de mensagens recentes
            const recentMessages = context.messages || [];
            // Preparar prompt para a IA
            const systemPrompt = get(automation, 'response.prompt', '');
            ...
            try {
                ...
                aiResponseText = await aiService.answerQuestion(context.incomingMessage.content, context.conversation.id, context.conversation.contactId, context.conversation.channelId);
                if (!aiResponseText) {
     ...
     * @returns Resultado da execução
     */
    async executeTrigger(automation, context) {
        try {
     ...
     * @returns Resultado da execução
     */
    async executeScheduled(automation, context) {
        try {
            // Verificar se há conversa
            if (!context.conversation) {
                return {
     ...
     */
    async sendMessageViaChannel(channel, recipient, content) {
        try {
