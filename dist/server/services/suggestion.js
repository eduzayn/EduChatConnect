 ...
 */
export async function suggestReply(message, conversationId) {
    try {
        ...
        const claudeSuggestions = await generateClaudeSuggestions(message, conversationHistory, conversation.channel);
        // 2. Sugestões baseadas em GPT (melhor para criatividade)
        const gptSuggestions = await generateGPTSuggestions(message, conversationHistory, conversation.channel);
 ...
 */
async function generateClaudeSuggestions(message, conversationHistory, channel) {
    try {
 ...
 */
async function generateGPTSuggestions(message, conversationHistory, channel) {
    try {
