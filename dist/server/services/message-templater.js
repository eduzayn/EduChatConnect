...
export class MessageTemplater {
     ...
     */
    processTemplate(templateString, context) {
        try {
            if (!templateString) {
                return '';
            }
    ...
    flattenContext(context) {
        ...
        flatContext.formatDate = (date, format = 'DD/MM/YYYY') => {
        ...
        flatContext.uppercase = (text) => text?.toUpperCase() || '';
        flatContext.lowercase = (text) => text?.toLowerCase() || '';
        flatContext.capitalize = (text) => {
            if (!text)
                return '';
     ...
     */
    processJSONTemplate(templateObject, context) {
        if (!templateObject) {
            return null;
        }
        try {
